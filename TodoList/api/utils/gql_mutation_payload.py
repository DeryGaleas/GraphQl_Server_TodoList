...
from TodoList.api.types import TaskError, InputTypeInterface, PayloadTypeInterface
...
def gql_mutation_payload(
    ...
    def inner(func):
        ...
        async def wrapper(self, input: input_type) -> payload_type:
            ...
            errors: List[TaskError] = await input.validate_and_get_errors()
            node: Optional[returned_type] = None
            if not errors:
                node: returned_type = await func(self=self, input=input)
            return payload_type(task_errors=errors, node=node)
